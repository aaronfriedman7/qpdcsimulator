...
def realization(params,rsd):
    Lcell = params.L
    try:
        Kvalues = np.load(workpath+"Kvals_pbc_L{0}_K1_{1}_K2_{2}_rsd{3}.npy".format(Lcell,params.K1,params.K2,rsd))
    except:
        Kvalues = np.random.uniform(params.K1,params.K2,(2,Lcell))*np.random.choice(np.array([-1.0,1.0]),(2,Lcell))
        np.save(workpath+"Kvals_pbc_L{0}_K1_{1}_K2_{2}_rsd{3}.npy".format(Lcell,params.K1,params.K2,rsd),Kvalues)
    try:
...
### odd nx and nz only, multiplied by c1x(mx) s1z(mz), or - this if for mz <0
...
### odd nx and nz only, multiplied by s1x(mx) c1z(mz), or - this if for mx <0
...
def XX_c(params,coeffs):
    ...
    for state in range(Size):
        for cell in range(Lcell):
            siteA = 2*cell + 1
            siteB = 2*((cell+1)%Lcell)
            vals.append(coeffs[0,cell])
            rows.append(state^(2**(2*Lcell-siteA-1)+2**(2*Lcell-siteB-1)))
            cols.append(state)
    return ss.coo_matrix((np.array(vals),(np.array(rows),np.array(cols))),shape=(Size,Size))
    ...
    for state in range(Size):
        ...
        for cell in range(Lcell):
            ...
            if scfg[2*cell+1] == scfg[2*cell+2]:
                ...
                vals.append(coeffs[1,cell])
            rows.append(state^( 2**(2*Lcell-siteA) + 2**(2*Lcell-siteA-1) + 2**(2*Lcell-siteB-1)+2**(2*Lcell-siteB-2)))
            cols.append(state)
    return ss.coo_matrix((np.array(vals),(np.array(rows),np.array(cols))),shape=(Size,Size))
    ...
    for state in range(Size):
        ...
        for cell in range(Lcell):
            ...
            if scfg[2*cell+1] == scfg[2*cell+2]:
                ...
                vals.append(-coeffs[1,cell]*1j)
                vals.append(-coeffs[1,cell]*1j)    
            rows.append(state^(2**(2*Lcell-siteA) + 2**(2*Lcell-siteA-1)))
            rows.append(state^(2**(2*Lcell-siteB-1) + 2**(2*Lcell-siteB-2)))
            cols.append(state)
            cols.append(state)
    return ss.coo_matrix((np.array(vals),(np.array(rows),np.array(cols))),shape=(Size,Size),dtype=complex)
...
def D1_Y_field(params,fields,FourierX,FourierZ):
    ...
    for mx in range(numterms):
        for mz in range(numterms):
            ...
            anti = FourierX[1,mx]*FourierZ[0,mz]*Y_sc(params,fields) + FourierX[0,mx]*FourierZ[1,mz]*Y_cs(params,fields) ## mx,mz > 0 ; - this if both < 0
            ...
            anti = FourierX[1,mx]*FourierZ[0,mz]*Y_sc(params,fields) - FourierX[0,mx]*FourierZ[1,mz]*Y_cs(params,fields) ## for mx > 0 , mz < 0; - this for opposite
            if not len((sym.nonzero())[0]) == 0 or len((anti.nonzero())[0]) == 0:
...
if __name__=="__main__":
    ...
    for rsd in range(params.ndis):
        ...
        try:
            D = ss.load_npz(workpath+"D0_pbc_"+jobhead(params,rsd)+".npz")
        except:
            D = D0(params,Ks,Js,FourierX,FourierZ)
            ss.save_npz(workpath+"D0_pbc_"+jobhead(params,rsd)+".npz",D) 
        
        try:
            D += ss.load_npz(workpath+"D1_pbc_"+jobhead(params,rsd)+".npz")
        except:
            D_1 = D1(params,Ks,Js,Bs,FourierX,FourierZ)
            ss.save_npz(workpath+"D1_pbc_"+jobhead(params,rsd)+".npz",D_1)
            D += D_1
        ...
        Gamma = G1(params,0.5*np.pi,0.5*np.pi,Ks,Js,Bs,FourierX,FourierZ).todense()
        print("Gamma1  (0) = ")
        print(Gamma)
        print("avg, std = {0} , {1} ".format(np.average(Gamma),np.std(Gamma)))
        print()
        
        Gamma = G1(params,1.5*np.pi,0.5*np.pi +np.pi/golden,Ks,Js,Bs,FourierX,FourierZ).todense()
        print("Gamma1  (1/2) = ")
        print(Gamma)
        print("avg, std = {0} , {1} ".format(np.average(Gamma),np.std(Gamma)))
        print()
